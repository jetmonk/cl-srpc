...
;;;
;;; See http://common-lisp.net/project/trivial-utf-8/ for ORIGINAL

;; see following copyright - taken and modified under terms of copyright
;; and altered slightly as noted
...
(in-package cl-srpc) ;; modified from original


;; package deleted from original
#|
 ==== useful functions ===
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defparameter *optimize-utf8* ;; altered from original
...
utf-8 representation.  BYTE-PAD-START and BYTE-PAD-END is empty
padding to put at start and end of output buffer."
  (declare (type string string)
	   (type (or null (unsigned-byte 20)) byte-pad-start byte-pad-end)
           #.*optimize-utf8*)
  (let ((buffer (make-array (+ (the fixnum (utf-8-byte-length string))
			       byte-pad-start byte-pad-end
                               (if null-terminate 1 0))
                            :element-type '(unsigned-byte 8)))
        (position byte-pad-start)
        (string (coerce string 'simple-string)))
...
;; new routine not in trivial-utf-i
(defun write-char-utf-8 (char output &key null-terminate) 
  ...
  (macrolet ((byte-out (byte)
    ...
    (as-utf-8-bytes char byte-out)))

;; new routine not in trivial-utf-i
(defun terpri-utf-8 (output)
  (write-char-utf-8 #\newline output))

(defun write-string-utf-8 (string output &key null-terminate)
  "Write a string to a byte-stream, encoding it as utf-8."
  (declare (type string string)
           (type stream output)
           #.*optimize-utf8*)
...
;; new routine not in trivial-utf-i
(defun write-utf-8-line (string output)
  (write-string-utf-8 string output)
  (terpri-utf-8 output)
  (force-output output))
...
the string it encodes." 
  (declare (type (simple-array (unsigned-byte 8))  bytes-in)
           (type fixnum start end)
           #.*optimize-utf8*)
  (loop :with bytes = (coerce bytes-in '(simple-array (unsigned-byte 8) (*)))
        :with buffer = (make-string (utf-8-string-length bytes :start start :end end)
...
(defun read-utf-8-string (input &key
				  null-terminated
				  stop-at-eof
				  stop-at-newline
...
max amount of characters or bytes to read. stop-at-lf means stop at
a #\newline or throw an error if no newline seen."
  ...
  (let ((buffer (make-array 4 :element-type '(unsigned-byte 8)))
    ...
    (loop
	 ...
	 (when stop-at-newline (error "No newline seen in string and STOP-AT-NEWLINE is set."))
	 (return))
      (let ((next-char (read-byte input (not stop-at-eof) :eof)))
	(when (or (eq next-char :eof)
		  (and stop-at-newline (eq next-char #.(char-code #\newline))) ;; added this
		  (and null-terminated (eq next-char 0)))
	  (when (and stop-at-newline
		     (not (eq next-char #.(char-code #\newline))))
	    (error "No newline seen in string and STOP-AT-NEWLINE is set."))
	  (return))
...
;; new function not in trivial-utf-8
(defun read-utf-8-line (input &key (char-length #xffff))
  (read-utf-8-string input :char-length char-length :stop-at-newline t))
